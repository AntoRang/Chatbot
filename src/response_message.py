...
def categorize_msg(message:str, prev_message:dict) -> str:
    ...
    catego = None
    if prev_message['type'] == 'crear_pedido':
        catego = 'crear_pedido2'
    elif prev_message['type'] == 'consulta_pedido':
...
def process_response(message: str, phone_n: str, google_client: DB.Client) -> str:
    ...
    # Avoid translation if message category like 
    # consulta_pedido2 or crear_pedido2 to preserve the org message
    if category == 'consulta_pedido2' or category == 'crear_pedido2': 
        message, org_lang = org_message, last_response['lang']
